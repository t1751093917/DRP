...
def main(cfg, device):
    ...
    # optimizer = build_adam_optimizer(net.parameters(), cfg.lr, eps=1e-3)
    ...
    #                                            keep_batchnorm_fp32=None, loss_scale='dynamic', verbosity=1)
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for it, sample in enumerate(train_loader):
            ...
            check_nan(x2, 'x2')
            check_nan(y, 'y')
            if check_nan(logits1, 'logits1') or check_nan(logits1, 'logits1') or check_nan(probs1, 'probs1') or check_nan(probs2, 'probs2'):
                all_nan = True
                for name, param in net.named_parameters():
                    all_nan = all_nan and check_nan(param, name)
                if all_nan:
                    exit(1)
            ...
            if epoch < cfg.warmup_epochs:
                ...
                # final loss
                loss = (1 - cfg.alpha) * loss_c + loss_o * cfg.alpha + 1e-8
            ...
            for name, param in net.named_parameters():
                if param.grad is not None:
                    # torch.clamp(param.grad, min=0, max=1)
                    check_nan(param.grad, name + '[grad]')
